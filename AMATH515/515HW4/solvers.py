...
def optimizeWithIP(x0, A, b, C, d, mu=1.0, rate=0.1, tol=1e-6, max_iter=1000):
    ...
    F[id_v] = np.diag(s)@v-mu*np.ones_like(v)
    F[id_s] = s+C@x-d
    # *************************************************************************
    dF = np.zeros((n+2*k, n+2*k))
    # *************************************************************************
    # TODO: Fill in dF
    # Hint: dF is a block-derivative of F(x, v, s) with respect to x, v and s
    # dF[id_x, id_x], dF[id_x, id_v], ...
    # *************************************************************************
    dF[id_x, id_x] = A.T@A
    dF[id_x, id_v] = C.T
    dF[id_v, id_s] = np.diag(v)
    dF[id_v, id_v] = np.diag(s)
    dF[id_s, id_s] = np.eye(s.size)
    dF[id_s, id_x] = C
    ...
    while err >= tol:
        ...
        # *********************************************************************
        # TODO: Adjust alpha such that (s + alpha*ds) is positive
        ind = np.where(ds < 0.0)[0]
        alpha = min(alpha, 0.99*np.min(-s[ind]/ds[ind]))
        # *********************************************************************
        ...
        # Hint: Do not need to update all parts of dF -- some are constants
        F[id_x] = A.T@(A@x-b)+C.T@v
        F[id_v] = np.diag(s)@v-mu*np.ones_like(v)
        F[id_s] = s+C@x-d
        dF[id_v, id_s] = np.diag(v)
...
def optimizeWithCP(x0, A, b, c, h, k, prox_ch, prox_k,
    ...
    while err >= tol:
        ...
        # update v
        v_new = prox_ch(v + alpha*(2.0*rx_new - rx), alpha)
